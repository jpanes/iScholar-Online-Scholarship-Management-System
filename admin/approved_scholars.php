       ...
       <?php
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-check"></i> Approved Scholars
                            </li>
                        </ol>
                    </div>
                </div>
        ...
        if(empty($_POST['search']))
         ...
         if((!empty($search)) && (isset($_POST['value']))){
                            ...
                            $query = "SELECT * FROM applicant WHERE FIRST_NAME = '".$search."' OR LAST_NAME = '".$search."' OR MIDDLE_NAME = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."' AND APPROVED = 'TRUE'";
                            }

                            elseif($_POST['value']=="filter_course"){
                            $query = "SELECT * FROM applicant Where COURSE = '".$search."' AND semester_granted ='".$_SESSION['SEM']."'  AND APPROVED = 'TRUE'";
                            }
                        
                            elseif($_POST['value']=="filter_student_id"){
                            $query = "SELECT * FROM applicant where ID_NUMBER = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'  AND APPROVED = 'TRUE'";
                            }

                            elseif($_POST['value']=="filter_family_income"){
                            $query = "SELECT * FROM applicant where FAMILY_INCOME = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'  AND APPROVED = 'TRUE'";
                            }

                            elseif($_POST['value']=="filter_year_level"){
                            $query = "SELECT * FROM applicant where YEAR_LEVEL = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'  AND APPROVED = 'TRUE'";
                            }

                            elseif($_POST['value']=="filter_All"){
                            $query = "SELECT * FROM applicant AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'  AND APPROVED = 'TRUE'";
                            }
                        

                        else{
                            $query = "SELECT * FROM applicant where LAST_NAME ='".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'  AND APPROVED = 'TRUE'";
                        }
        }

        else {$query = "SELECT * FROM applicant WHERE school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'  AND APPROVED = 'TRUE'";}

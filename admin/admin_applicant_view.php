                    ...
                    <div class="col-lg-12">
                       <br>
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-edit"></i> Applicants
                            </li>
                        </ol>
                    </div>
                </div>
...
---------------------------------------------->
        ...
        if(empty($_POST['search']))
         ...
         if((!empty($search)) && (isset($_POST['value']))){
                            ...
                            $query = "SELECT * FROM applicant WHERE FIRST_NAME = '".$search."' OR LAST_NAME = '".$search."' OR MIDDLE_NAME = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'";
                            }

                            elseif($_POST['value']=="filter_course"){
                            $query = "SELECT * FROM applicant Where COURSE = '".$search."' AND semester_granted ='".$_SESSION['SEM']."'";
                            }
                        
                            elseif($_POST['value']=="filter_student_id"){
                            $query = "SELECT * FROM applicant where ID_NUMBER = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'";
                            }

                            elseif($_POST['value']=="filter_family_income"){
                            $query = "SELECT * FROM applicant where FAMILY_INCOME = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'";
                            }

                            elseif($_POST['value']=="filter_year_level"){
                            $query = "SELECT * FROM applicant where YEAR_LEVEL = '".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'";
                            }

                            elseif($_POST['value']=="filter_All"){
                            $query = "SELECT * FROM applicant AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'";
                            }
                        

                        else{
                            $query = "SELECT * FROM applicant where LAST_NAME ='".$search."' AND school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'";
                        }
        }

        else {$query = "SELECT * FROM applicant WHERE school_year ='".$_SESSION['SY']."' AND semester_granted ='".$_SESSION['SEM']."'";}
                                ...
                                <table class="table table-hover" id="applicants">
                                        ...
                                        <thead>
                                            <tr>    
                                                ...
                                                <th>Course & Year</th>
                                                <th>Family Income</th>
                                                <th>Residence & Ownership Type</th>
                                            </tr>
                                        </thead>
                                    <tbody>
                                                <?php
                                                ...
                                                <td><?php echo $row['RES_TYPE'].' & '.$row['OWN_TYPE']?></td>
                                                <td><a href="admin_view_appform.php?id=<?php echo $row['STUDENT_ID']; ?> " class="btn btn-success" role="button" >Full Details</a></td>
